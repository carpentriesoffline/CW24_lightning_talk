...
\begin{frame}
	...
	\begin{columns}[t]
		\begin{column}{0.6\textwidth} % Left column width
			...
			\begin{itemize}
   				\item Inconsistent internet access, which disproportionately impacts individuals with lower incomes, creates challenges for data science workshop participants, individual learners, and professionals aiming to learn or utilize data science and software development skills. 
       
				\item OfflineDataSci is a Python package with a command-line interfaces that is designed for both maintaining local teaching servers and for local use by individual learners and data science practitioners.
				
				\item Downloads lesson materials for Software-, Data, and Library Carpentries, along with the latest installers for essential tools such as \textbf{R}, \textbf{Python}, and \textbf{Rstudio}. 
				
				\item Creates partial local mirrors of the R (miniCRAN) and Python (PyPi-mirror) package repositories, containing data science packages for data manipulation, visualization, and analysis.
			\end{itemize}
		\end{column}
